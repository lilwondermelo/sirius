...
class App {
    ...
    public function uploadArrivalExcel() {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            $this->error = 'Ошибка загрузки файла: ' . $file['error'];
            return false;
        }
        ...
        if ($fileExtension !== 'xlsx') {
            $this->error = 'Неверный формат файла. Пожалуйста, загрузите файл в формате .xlsx. Вы можете просто пересохранить ваш .xls файл в новый формат в программе Excel.';
            return false;
        }

        $filePath = $file['tmp_name'];

        require_once $this->root . '/server/lib/SimpleXLSX.php';
        ...
        if ($xlsx = SimpleXLSX::parse($filePath)) {
            ...
            // A more robust way to find all product rows, assuming multiple tables in one sheet
            foreach ($rows as $rowIndex => $row) {
                // Heuristic to find a header row for item data
                if (isset($row[0]) && trim($row[0]) === 'А' && isset($row[1]) && trim($row[1]) === '1') {
                ...
                // Stop processing if we reach the end of a block
                if ( (isset($row[0]) && strpos(trim($row[0]), 'Всего к оплате') !== false) || empty(array_filter($row)) ) {
                    // Could reset start_processing here if multiple independent tables are possible
                    // $start_processing = false; 
                    continue;
                }

                // Check if it looks like a product row (must have a number in the second column)
                if (isset($row[1]) && is_numeric(trim($row[1]))) {
                    $vendor_code = trim($row[0] ?? '');
                    $name = trim($row[4] ?? '');
                    $quantity = (float)str_replace(',', '.', trim($row[10] ?? '0'));
                    $purchase_price = (float)str_replace(',', '.', trim($row[12] ?? '0'));

                    if (empty($vendor_code) || empty($name) || $quantity <= 0) {
                        continue;
                    }

                    // Find product_id by vendor_code
                    $query = sprintf("SELECT id FROM list_items WHERE vendor_code = '%s'", addslashes($vendor_code));
                    $dataSource = new DataSource($query);
                    $productData = $dataSource->getData();
                    
                    $product_id = null;
                    if ($productData && isset($productData[0]['id'])) {
            ...
            $this->error = 'Не удалось разобрать файл Excel: ' . SimpleXLSX::parseError();
            return false;
        }
    }
}
